...
export async function GET(request: NextRequest) {
  ...
  const trimmedUrl = nextAuthUrl.trim(); // 개행문자 제거
  ...
  const config = {
    ...
    NEXTAUTH_URL_TRIMMED: trimmedUrl,
    NEXTAUTH_URL_HAS_NEWLINE: nextAuthUrl.includes('\n'),
    NEXTAUTH_URL_LENGTH: nextAuthUrl.length,
    REDIRECT_URI_ORIGINAL: nextAuthUrl + '/api/auth/social/kakao/callback',
    REDIRECT_URI_TRIMMED: trimmedUrl + '/api/auth/social/kakao/callback',

-- Phase 1: 다중 담당자 업무 할당 알림 시스템 구현
-- 업무 할당 시 담당자별 알림 생성
...
BEGIN
    ...
    SET
        ...
        metadata = metadata || jsonb_build_object('reassigned_at', NOW())
...
CREATE INDEX IF NOT EXISTS idx_task_notifications_expires_at ON task_notifications(expires_at) WHERE expires_at > NOW();

-- 9. 테스트 데이터 확인
SELECT
    'Phase 1 스키마 업데이트 완료' as status,
...
-- 성공 메시지
SELECT 'Phase 1: 다중 담당자 업무 할당 알림 시스템 준비 완료' as result;
...
interface ContractBusinessTemplateProps {
  data: {
...
export default function ContractBusinessTemplate({ data }: ContractBusinessTemplateProps) {
  ...
  const additionalCostTotal = data.additional_cost + additionalCostVat

  // 네고금액 부가세 계산
  const negotiationCostVat = Math.round(data.negotiation_cost * 0.1)
  const negotiationCostTotal = data.negotiation_cost + negotiationCostVat

  // 총 공급금액
  const totalSupplyAmount = data.government_notice_price + data.additional_cost + data.negotiation_cost

  // 총 부가세 (사업비 부가세 + 추가비용 부가세 + 네고금액 부가세)
  const totalVat = businessVat + additionalCostVat + negotiationCostVat

  // 총 입금액 (IoT 입금액 + 추가비용 입금액 + 네고금액 입금액)
  const totalDeposit = depositAmountIot + additionalCostTotal + negotiationCostTotal
  ...
  return (
    <div className="bg-white p-8 max-w-4xl mx-auto" style={{ fontFamily: 'Malgun Gothic, sans-serif' }}>
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-sm text-gray-600 mt-1">(사업장 보관용)</p>
      </div>
      ...
      <div className="mb-6">
        ...
        <p className="mb-3 leading-relaxed pl-4">
          ...
          <span className="text-blue-600"> (자체 투자비와 공사비 및 자재비는 공사전 입금을 원칙으로 함.)</span>
        </p>
        ...
        <table className="w-full border border-black mb-3" style={{ borderCollapse: 'collapse' }}>
          ...
          <tbody>
            ...
            <tr>
              <td className="border border-black px-3 py-2 text-sm text-center">기타 비용</td>
              <td className="border border-black px-3 py-2 text-sm text-right">
                {data.additional_cost.toLocaleString()}
              </td>
              <td className="border border-black px-3 py-2 text-sm text-right text-center">-</td>
              <td className="border border-black px-3 py-2 text-sm text-right">
                {additionalCostVat.toLocaleString()}
              </td>
              <td className="border border-black px-3 py-2 text-sm text-right text-center">-</td>
              <td className="border border-black px-3 py-2 text-sm text-right text-blue-600">
                {additionalCostTotal.toLocaleString()}
              </td>
            </tr>
            <tr>
              <td className="border border-black px-3 py-2 text-sm text-center">협의 금액</td>
              <td className="border border-black px-3 py-2 text-sm text-right">
                {data.negotiation_cost.toLocaleString()}
              </td>
              <td className="border border-black px-3 py-2 text-sm text-right text-center">-</td>
              <td className="border border-black px-3 py-2 text-sm text-right">
                {negotiationCostVat.toLocaleString()}
              </td>
              <td className="border border-black px-3 py-2 text-sm text-right text-center">-</td>
              <td className="border border-black px-3 py-2 text-sm text-right text-blue-600">
                {negotiationCostTotal.toLocaleString()}
              </td>
            </tr>
        ...
        <div className="text-xs text-center space-y-1">
          ...
          <p className="text-blue-600 font-semibold">
            (입금시 보조금 자부담+부가세 비용과 / 기타비용 따로 입금, 자부담 입금 확인증 필요)
          </p>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <p className="pl-4 leading-relaxed">
          ...
          {' ~ '}
          <span className="font-semibold">{formatDate(endDate)}</span> 이내 공사를 완료하며,
          설치 일정은 상호 협의하여 정한다.
        </p>
      </div>

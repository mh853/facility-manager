-- Phase 1: 다중 담당자 업무 할당 알림 시스템 구현 (최종 수정 버전)
-- IMMUTABLE 함수 오류 및 current_time 변수명 충돌 해결
...
BEGIN
    ...
    LOOP
        ...
        RAISE NOTICE '업무 ID %: assignee "%"를 assignees 배열로 변환', task_record.id, task_record.assignee;
    END LOOP;
END $$;

-- 10. 테스트 데이터 확인
SELECT
    'Phase 1 스키마 업데이트 완료 (최종 수정됨)' as status,
...
-- 성공 메시지
SELECT 'Phase 1: 다중 담당자 업무 할당 알림 시스템 준비 완료 (최종)' as result;